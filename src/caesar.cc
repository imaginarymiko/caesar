...
int letterToIndex(char letter) {
    ...
    if (letter >= 'a' && letter <= 'z') return static_cast<int>(letter - 'a');
    else if (letter >= 'A' && letter <= 'Z') return static_cast<int>(letter - 'A');
    else return -1;
}
...
// Accessors
std::string CaesarText::getText() const { return this->text; }

void CaesarText::setText(const std::string& text) { this->text = text; }

// Methods
void CaesarText::print() const { std::cout << this->text << std::endl; }
...
float CaesarText::getEntropy() const {
    ...
    for (char letter: this->text) {
        if (letterToIndex(letter) != -1) sum += std::log(ENGLISH_FREQUENCIES[letterToIndex(letter)]);
        else ++ignored;
    }
...
std::string CaesarText::shiftText(int shift) const {
    ...
    std::string newText = "";
    for (char letter: this->text) {
        if ('A' <= letter && letter <= 'Z') newText += static_cast<char>((letter - 'A' + shift) % 26 + 'A');
        else if ('a' <= letter && letter <= 'z') newText += static_cast<char>((letter - 'a' + shift) % 26 + 'a');
        else newText += letter;
    }
    return newText;
}
std::vector<int> generateKey(const std::string& text) {
    ...
    for (char letter: text) {
        if ('A' <= letter && letter <= 'Z') key.push_back(letter - 'A');
        else if ('a' <= letter && letter <= 'z') key.push_back(letter - 'a');
    }
    return key;
}
...
void CaesarText::fillResults(std::vector<std::tuple<int, std::string, double>>& results) const {
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        results.push_back(std::make_tuple(i, tempText, CaesarText(tempText).getEntropy()));
    }
    std::sort(results.begin(), results.end(), 
        [](const std::tuple<int, std::string, double>& a, const std::tuple<int, std::string, double>& b) -> bool {
